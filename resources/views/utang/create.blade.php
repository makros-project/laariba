...
@section('content_header')
    <h1>{{ isset($utang) ? 'Edit Utang' : 'Tambah Utang' }}</h1>
@endsection
...
@section('content')
    <div class="card">
        <div class="card-body">
            ...
            <form action="{{ isset($utang) ? route('utang.update', $utang) : route('utang.store') }}" method="POST">
                @csrf
                @if(isset($utang))
                    @method('PUT')
                @endif
                ...
                <div class="form-group">
                    ...
                    <input type="date" name="tanggal_jatuh_tempo" id="tanggal_jatuh_tempo" class="form-control" value="{{ old('tanggal_jatuh_tempo', $utang->tanggal_jatuh_tempo ?? '') }}">
                </div>
                ...
                <button type="submit" class="btn btn-primary">{{ isset($utang) ? 'Update' : 'Simpan' }}</button>
